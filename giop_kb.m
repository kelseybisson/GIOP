function [x,mapg,maph,madg,mbbp,mrrs] = giop_kb(wl,rrs,chl,gopt)
...
% adapted by kelsey bisson, 28 feb 2019

mapg = -999; maph = -999; madg = -999; mbbp = -999; mrrs = -999;
...
% define eta (default to QAA) (bbp slope)
...
if isfield(gopt,'aph') == 1
	...
	elseif strcmp(gopt.aph,'gsm')
		% from GSM (Maritorena et al. 2002)
		gopt.aphs = interp1([412,443,490,510,555,670],[0.006650,0.05582,0.02055,0.0191,0.010150,0.01424],wl,'pchip');
	...
	elseif strcmp(gopt.aph,'chase')
%		% from ali chase.)
        gopt.aphs = get_chase_ap(chl,wl);
