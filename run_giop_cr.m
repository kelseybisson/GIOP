...
figure(102)
plot(wl, gopt.aw,'b-',wl,aph,'g-',wl,adg,'c-'),ylabel('a(\lambda) (m^{-1})'),xlabel('Wavelength (nm)'),legend('a_w','a_{phyt}','a_{cdm}')
figure(103)
plot(wl,gopt.bbw,'b-',wl,bbp,'c-'),ylabel('b_b(\lambda) (m^{-1})'),xlabel('Wavelength (nm)'),legend('b_{bw}','b_{bp}')
figure(1)
...
%uncomment the following three lines to run LMI
% gopt.inv = 'lmi';
% t1 = giop(wl,rrs,oc(1),gopt);
% t1
